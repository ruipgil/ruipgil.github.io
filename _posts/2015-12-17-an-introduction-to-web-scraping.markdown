...
In practice, and formal definitions apart, web scraping is the act of extracting information of a web page. What's this information to be scraped? I'll let you be the judge of that, as it can range from simple text, to images, to code, to values hidden from the eyes of people (values in runtime variables).

There's a number of ways to achieve it, more specially a number of libraries or tools to do it.
...
scraperjs.StaticScraper.create('http://ruipgil.com{% post_url 2015-06-23-web-scraping %}')
  .scrape(function($) {
...
In *scraperjs* the usage difference between the two types of scrapers lies on the scope that the callback of the scrape function has access to. In the dynamic scraper you can't access functions in you code as you would normally, but you can use browser specific function and objects without declaring them (like ``` document.querySelectorAll('a') ```).

``` js
var scraperjs = require('scraperjs');

scraperjs.DynamicScraper.create('http://ruipgil.com/2015/06/23/web-scraping.html')
  .scrape(function($) {
    return $('a').map(function() {
      return $(this).attr('href');
    }).get();
...
router.route('http://ruipgil.com{% post_url 2015-06-23-web-scraping %}', function(found, result) {
  if( found ) {
...
``` scraperjs http://ruipgil.com{% post_url 2015-06-23-web-scraping %} --attr href --selector a --static ```
...
scraperjs
  ...
  .get({
    url: 'http://ruipgil.com{% post_url 2015-06-23-web-scraping %}',
    header: {
